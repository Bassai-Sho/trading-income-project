...
-------------
  ...
  The engine reads trading_quant_toolkit_v2_1.py from the same directory.
"""
...
try:
    ...
    try:
        ...
        import trading_quant_toolkit_v2_4 as tk  # type: ignore  # type: ignore
    TOOLKIT = True
except ImportError:
    TOOLKIT = False
    logging.warning("trading_quant_toolkit_v2_1.py not found — using engine's built-in calculations")
...
CONFIG: dict[str, Any] = {
    ...
    "telegram_chat_id":  "",             # optional: your Telegram chat_id
...
class PaperAccountDB:
    ...
    -- Account equity tracking (compounding, per-variant)
    ...
    def init_account(self, starting_balance: float) -> None:
        """Initialise the account record on first startup."""
        with self._conn() as conn:
            conn.execute(
    ...
    def get_current_equity(self, starting_balance: float | None = None) -> float:
        """
        Running equity = starting_balance + sum of all closed P&L (net of costs).
        Uses DB accounts table if initialised; falls back to starting_balance arg.
        """
    ...
    def update_account_equity(self, current_equity: float,
                               ror_pct: float | None = None) -> None:
        """Update the accounts row with current equity and risk-of-ruin."""
        with self._conn() as conn:
            conn.execute(
...
def compute_entry_params(ctx: dict, db: PaperAccountDB, cfg: dict) -> dict:
    ...
    orb_low      = ctx["orb_low"]
    # Use CURRENT equity (compounding), not fixed starting balance
...
def _exit_position(pos: dict, exit_price: float, reason: str,
    ...
    # Use current equity for P&L calculation (compounding)
    current_eq = db.get_current_equity(cfg["account_balance"])
    base_risk  = current_eq * cfg["risk_pct"]
    # Net actual_r of round-trip cost (spread + market impact)
    cost_r     = pos.get("cost_r") or 0.016   # fallback 1.6% of risk
...
def _notify(cfg: dict, message: str, level: str = "INFO") -> None:
    """
    Post a notification to Discord and/or Telegram.
    ...
    if not webhook and not tg_token:
        return
        ...
        if tg_token and tg_chat:
            try:
                ...
                tg_payload = _json.dumps({
                    "chat_id": tg_chat,
                    "text": f"{emoji} *{level}* [{now_est}]  {message}",
                    "parse_mode": "Markdown"
                }).encode()
...
class TradingEngine:
    ...
    def __init__(self, cfg: dict) -> None:
        ...
        self.db.init_account(cfg["account_balance"])  # sets starting_balance once
        self.running  = False
        self.session: dict[str, Any] = self._fresh_session()
        self._daily_df = None   # cached daily OHLCV for journal context
    ...
    def _tick(self) -> None:
        ...
        if now.time() > self.cfg["session_end"]:
            ...
            if pos:
                ...
                last_close = float(df_5m["Close"].dropna().iloc[-1]) if not df_5m.empty else pos["entry_price"]
                vwap = _vwap(df_5m)
            ...
            # Trigger LLM session analyser in background
        ...
        log.info("[%s] %s | ORB:%s VWAP:%s Gate:%s | %s",
                  ...
                  ctx["gate_final"], ctx.get("reason", "")[:60] if isinstance(ctx.get("reason"), str)
    ...
    def _trigger_analyser(self, session_date: str) -> None:
        ...
        analyser = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                                "session_analyser.py")
        if not os.path.exists(analyser):
            log.info("session_analyser.py not found — skipping LLM analysis")
            return
        cmd = [_sys.executable, analyser,
               "--date", session_date,
               "--db",   self.cfg["db_path"],
               "--model", self.cfg.get("llm_model", "local-model"),
               "--url",   self.cfg.get("llm_base_url", "http://localhost:1234/v1")]
        if self.cfg.get("discord_webhook"):
            cmd += ["--discord", self.cfg["discord_webhook"]]
        if self.cfg.get("telegram_token"):
            cmd += ["--telegram-token",   self.cfg["telegram_token"],
                    "--telegram-chat-id",  self.cfg.get("telegram_chat_id","")]
        try:
            subprocess.Popen(cmd, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
            log.info("Session analyser launched for %s", session_date)
            _notify(self.cfg, f"Session complete — LLM analysis running for {session_date}", "INFO")
        except Exception as exc:
            log.warning("Could not launch session analyser: %s", exc)
    ...
    def _persist_session(self, today: str) -> None:
        ...
        # Update account equity and check risk of ruin
        current_equity = self.db.get_current_equity(self.cfg["account_balance"])
        ror_pct = None
        rs = self.db.rolling_stats(50)
        if rs.get("n", 0) >= 20 and rs.get("wr") and rs.get("aw") and rs.get("al"):
            try:
                from monte_carlo_extended import risk_of_ruin
                ror = risk_of_ruin(
                    win_rate=rs["wr"], avg_win_r=rs["aw"], avg_loss_r=rs["al"],
                    risk_pct=self.cfg["risk_pct"],
                    current_equity=current_equity,
                    n_sims=2000, n_trades_horizon=500,
                )
                ror_pct = ror.get("p_ruin", 0) * 100
                if not ror.get("safe", True):
                    log.warning("RISK OF RUIN: P(ruin)=%.1f%% — review sizing", ror_pct)
                    _notify(self.cfg,
                            f"RISK ALERT: P(>50% drawdown)={ror_pct:.1f}% "
                            f"after {rs['n']} trades. Current equity=£{current_equity:,.0f}. "
                            f"Review position sizing.", "HALT")
            except Exception as e:
                log.debug("RoR check failed: %s", e)
        self.db.update_account_equity(current_equity, ror_pct)
        log.info("EOD equity: £%,.2f  RoR: %s", current_equity,
                 f"{ror_pct:.1f}%" if ror_pct is not None else "N/A (< 20 trades)")

        # Run SessionLearner and persist report once per session
        # (only after at least one trade or 5+ decisions logged)
        decisions = self.db.get_decisions(today, limit=200)
        trades    = self.db.get_session_trades(today)
        if TOOLKIT and hasattr(tk, "SessionLearner") and (decisions or trades):
            try:
                learner = tk.SessionLearner(decisions, trades)
                report  = learner.generate_refinement(avg_atr=0.80, risk_amount=100.0)
                self.db.save_session_learning(today, report)
                if report.get("action_items"):
                    for item in report["action_items"]:
                        log.info("LEARNING: %s", item[:120])
            except Exception as exc:
